...
if model_loaded:
    ...
    for col in input_df.columns:
        ...
        perturbed_full   = engineer_features(perturbed_df)
...
with tab_clinical:
    ...
    st.dataframe(display_df, use_container_width=True)    --amber:      #F5A623;
    --red:        #F05252;
    --blue:       #4F8EF7;
    --text:       #E8EDF7;
    --muted:      #7A8BAD;
...
if model_loaded:
    ...
    risk_score   = model.predict_proba(input_scaled)[0][1]
    # Per-feature importance (SHAP-style approximation via perturbation)
    baseline = model.predict_proba(input_scaled)[0][1]
    feature_impacts = {}
    for col in input_df.columns:
        perturbed = input_scaled.copy()
        col_idx   = list(input_df.columns).index(col)
        perturbed[0][col_idx] += 0.5
        perturbed_score = model.predict_proba(perturbed)[0][1]
        feature_impacts[col] = round((perturbed_score - baseline) * 100, 2)
else:
    # Demo mode — deterministic sim from inputs
    score_raw  = (
